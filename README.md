...
(Parakeet) engine by default, with an optional Streaming mode powered by the
Nemotron/Parakeet Core ML 560 ms model via FluidAudio — no Python, no network
at runtime.

## Building from source

This repo holds **only the Budgie app**. The build embeds three things too
large for git — the CrispASR speech engine, the standard Parakeet GGUF model
and the Streaming Core ML model — so a fresh clone needs them set up once. They
...
huggingface-cli download FluidInference/nemotron-speech-streaming-en-0.6b-coreml \
  --include "nemotron_coreml_560ms/**" \
  --local-dir models
```

This produces `models/nemotron_coreml_560ms/`, the balanced 0.56 s Core ML
variant used when **Settings... -> General -> Streaming mode** is enabled.
...
Produces a **self-contained** `Budgie.app` — the `crispasr` engine, its dylibs,
the standard speech model and the Streaming Core ML model are all bundled
inside, with library paths rewritten to `@executable_path`, so the app runs on
...
`Sources/Budgie/Config.swift` resolves the engine and model from inside the
app bundle at runtime (`Contents/MacOS/crispasr` and the bundled
`parakeet-tdt-0.6b-v3-q4_k.gguf`, plus `nemotron_coreml_560ms/` for Streaming
mode). When run outside a bundle — e.g. via `swift run` during development —
it falls back to the build-tree copies in this repo. To change models or quant,
edit the dylib/model list in `build.sh` (and the resource names in
`Config.swift`) and rebuild.
