...
(Parakeet) engine — no Python, no network.

## Building from source

This repo holds **only the Budgie app**. The build embeds two things too large
for git — the CrispASR speech engine and the Parakeet model — so a fresh clone
needs them set up once. Both live *inside* the repo folder and are git-ignored,
so once set up everything is self-contained.
...
The two steps below are run from inside this `budgie/` folder, producing:
...
Produces a ~476 MB **self-contained** `Budgie.app` — the `crispasr` engine,
its dylibs and the speech model are all bundled inside, with library paths
rewritten to `@executable_path`, so the app runs on any Apple-silicon Mac
with nothing else installed. Requires the Swift toolchain (Xcode CLT) to build.
...
`parakeet-tdt-0.6b-v3-q4_k.gguf`). When run outside a bundle — e.g. via
`swift run` during development — it falls back to the build-tree copies in
this repo. To change models or quant, edit the dylib/model list in `build.sh`
(and the resource name in `Config.swift`) and rebuild.
